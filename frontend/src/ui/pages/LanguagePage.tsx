import { I18N, setLocale, useLocale } from '../../i18n/localize'

export function LanguagePage(props: { onBack: () => void }) {
      ...
      <div className="mx-auto max-w-3xl px-6 py-10">
        <div className="flex items-center justify-between gap-3">
          <div>
            <div className="text-xl font-black tracking-widest text-slate-100">
              <I18N k="language.title" default="Language" />
            </div>
            <div className="mt-1 text-xs font-medium tracking-[0.35em] text-slate-400">
              <I18N k="language.choose" default="Choose locale" />
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="mt-8 rounded-md border border-black/60 bg-black/20 p-5 shadow-[0_2px_0_0_rgba(0,0,0,0.65)]">
          <div className="text-sm font-bold tracking-wide text-slate-100">
            <I18N k="language.current" default="Current" />: <span className="text-slate-200">{locale}</span>
          </div>
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2">
            {supportedLocales.map((l) => (
              <button
                ...
                onClick={() => setLocale(l)}
                className={[
                  'rounded-md border border-black/60 bg-gradient-to-b px-4 py-3 text-left text-sm font-bold tracking-wide shadow-[0_2px_0_0_rgba(0,0,0,0.65)] active:translate-y-[1px] active:shadow-[0_1px_0_0_rgba(0,0,0,0.65)]',
                  l === locale
                    ? 'from-emerald-300/15 to-emerald-900/25 text-emerald-50'
                    : 'from-slate-200/10 to-black/30 text-slate-100 hover:from-slate-200/15 hover:to-black/35'
                ].join(' ')}
              >
                <I18N k={`locale.${l}`} default={l} />
              </button>
            ))}
          </div>
        </div>
